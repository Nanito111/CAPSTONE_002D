    ...
    try{
        ...
        const apiResponse = new Response(JSON.stringify(data), {
            ...
            headers: {
                ...
                'Set-Cookie': `${tokenTypeCookie}, ${accessTokenCookie}`
            }
        });
        return apiResponse;

        //const apiResponse = new Response(JSON.stringify(data), { status: response.status, headers: { 'Content-Type': 'application/json' } });
        return apiResponse;
    }
    catch (error) {
